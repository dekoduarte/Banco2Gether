...
class SistemaInterno {
	...
	public static void escolheOpcao(Usuario usuario, OpcoesMenu opcao, TipoContas tipo) {
		...
		if (opcao == OpcoesMenu.INICIO) {
			System.out.println(
					"-- (1) = Entrar como Cliente (Conta Particular)\n-- (2) = Entrar do Menu Banco (Ger�nciar)");
			int op = scan.nextInt();
			switch (op) {
			case 1:
	...
	public static void main(String[] args) {
		...
		/*
		 * if (usuarioLogado != null) if (usuarioLogado instanceof Cliente) { Cliente
		 * cliente = (Cliente) usuarioLogado;
		 * 
		 * if(cliente.getConta().getTipo_conta()==TipoContas.POUPANCA) {
		 * System.out.println("\nTITULAR: "+cliente.getNome()+" | CONTA "+cliente.
		 * getConta().getTipo_conta()+" - N�MERO: "+cliente.getConta().getConta()
		 * +" - AG�NCIA: "+cliente.getConta().getAgencia()+" |"); System.out.
		 * println("\t------ Digite a Opera��o Desejada ------\n\n-- (1) = Opera��es Banc�rias\n-- (2) = Informa��es da Conta\n-- (3) = Sair\n"
		 * );
		 * 
		 * int op = scan.nextInt(); switch (op) { case 1:
		 * escolheOpcao(OpcoesMenuCliente.OPERACOES); break; case 2:
		 * escolheOpcao(OpcoesMenuCliente.INFORMACOES); break; default:
		 * escolheOpcao(OpcoesMenuCliente.INVALIDA); }
		 * 
		 * /*cliente.getConta().sacar(20); cliente.getConta().depositar(100);
		 * 
		 * System.out.println(cliente.relatorioTributacaoContaCorrente());
		 * 
		 * }else if(usuarioLogado instanceof Gerente) { Gerente gerente = (Gerente)
		 * usuarioLogado;
		 * 
